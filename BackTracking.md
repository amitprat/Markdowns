...
- Use backtrack to try put every number in [1,9] at every empty position if it safe to put and then keep trying until we solve the entire board or we reach a position where we can't put any number safely. At that step, we backtrack and reset the values in stack unbind. After that, try next number on that position.
- For validating the sudoku board:
  - We can use naive approach for to check for each row, col and block for all elements in array.
    T = O(N\*N\*N) ,where N - size of the board. N\*N - steps to iterate entire board, and extra N to validate row, col and block.
  - We can optimize this using hashset. We still have to iterate the entire board(N\*N), but while iterating the board, we keep inserting the key(identifier) that 'this particular number' has been seen in 'this' row, 'this' column and 'this' block. In a valid sudoku board, we shouldn't have any key collision but if board is invalid then, we have duplicate keys and then we can fail at that step.
    T = (N\*N), where N - size of board, hashset lookup = O(1)
